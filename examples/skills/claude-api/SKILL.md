...
> **Adapting this skill for Ada.** Ada is a local-first .NET app: her default brain is local (Ollama/ONNX) and Anthropic is an *optional* cloud provider. Use this skill whenever you write code that calls the Claude/Anthropic API — Ada's own local-Ollama default does not suppress it.
> - **API keys:** never hardcode a key or tell the user to `export ANTHROPIC_API_KEY`. Ada keeps provider keys in her encrypted vault (Settings → Cloud providers); generated code should read the key from the environment/secret at runtime and never print it.
> - **C# is first-class here.** Ada is .NET, so when she writes Anthropic code for herself, prefer the `csharp/` track (`csharp/claude-api.md` has the authoritative SDK surface — `dotnet add package Anthropic`). A C# snippet can run via Ada's `run_code` tool (a .NET 10 file-based app, `#:package Anthropic`). Caveat: `run_code` runs `--network none`, so a *live* call needs network — use `run_code` for code Ada hands to the user, or run it host-side, rather than expecting the API call to complete inside the sandbox.
> - **Out of scope for Ada:** the **Managed Agents** sections (Anthropic's hosted-agent product) and their `shared/managed-agents-*.md` files were removed in this import — ignore those sections and focus on single calls, tool use, streaming, caching, and token counting.
